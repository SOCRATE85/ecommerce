...
import Title from './AddAttributeSet/Title';
...
const EditAttributeSet = () => {
    ...
    const {loading: loadingDetail, attributeset} = useSelector(state => state.attributesetDetails);
    ...
    useDeepCompareEffect(() => {
        const matched = (attribute) => {
           ...
           for (let _key in AttributeGroup) {
               const find =  AttributeGroup[_key].attributes.find(item => item._id === attributeId);
               if(find) {
                    count++;
               }
           }
       ...
       const loadData = () => {
            ...
            for(let key in attributes) {
                ...
                attributesList.push({
                    ...
                    is_done: flags === 0 && attributes[key].is_user_defined === true ? false : true,
                ...
                if(newAttributeGroup.length > 0) {
                    for(let key in newAttributeGroup) {
                        if(newAttributeGroup[key].title === 'General') {
                            newAttributeGroup[key].attributes = attributesList.filter(item => item.is_done === true && item.moved === false);
                        }
                    };
                }
    ...
    const addAttributeGroupHandler = () => {
        const newAttributeGroup = [...AttributeGroup];
        const checked = newAttributeGroup.filter((group) => group.title === groupName);
        if(checked.length !== 0 || groupName === '') {return;}
    ...
    const addAttributeToAttributeSetHandler = (attribute) => {
        ...
        for(let key in newAttributeGroup) {
            if(newAttributeGroup[key].index === selectedGroup)  {
                ...
                const checked = selectedAttributeGroup.attributes.filter((item) => item._id === attribute._id);
                if(checked.length > 0) {
                    return;
                }
    ...
    const moveOneGroupToAnother = (attribute, sourceIndex) => {
        ...
        //target
        let newTargetAttributeGroup = attributeGroup.find(item => item.index === targetIndex);
        const targetAttributes = [...newTargetAttributeGroup.attributes];
        const checked = targetAttributes.filter((item) => item._id === attribute._id);
        if(checked.length > 0) {
            newTargetAttributeGroup.attributes = targetAttributes
        } else {
            newTargetAttributeGroup.attributes = [...targetAttributes, {...attribute, moved: true}];
        }
        updateAttributeGroup.push(newTargetAttributeGroup);

        // source
        let newSourceAttributeGroup = attributeGroup.find(item => item.index === sourceIndex);
    ...
    const deleteAttributeGroupHandler = () => {
        ...
        const selectedAttributeAttributeGroup = AttributeGroup.find((item) => item.index === selectedGroup);
        const attributeOfSelectedAttributeGroup = selectedAttributeAttributeGroup.attributes;
        const noneSelectedAttributeGroup = AttributeGroup.filter((item) => item.index !== selectedGroup);
        
        const newCompletedTodos = [...CompletedTodos];
        const updateCompletedTodos = [];
        for(let key in newCompletedTodos) {
            for(let _key in attributeOfSelectedAttributeGroup) {
                if (newCompletedTodos[key]._id === attributeOfSelectedAttributeGroup[_key]._id) {
    ...
    const deleteAttributeFromGroupHandler = (attribute, index) => {
        const newAttributeGroup = [...AttributeGroup];
        const selectedGroupWithItem = newAttributeGroup.find(item => item.index === index);
        selectedGroupWithItem.attributes = selectedGroupWithItem.attributes.filter(item => item._id !== attribute._id);//[...selectedGroupWithItem.attributes];
    ...
    const openGroupForm = () => {
        return (<div className="groupForm">
            ...
            <div className="actions">
                ...
                <div className="done"><Done onClick={() => addAttributeGroupHandler()} /></div>
            </div>
        </div>);
    }
    return (<>
            ...
            <div className='attributeSetContainer'>
                ...
                <div className="addAttributeset">            
                    <div className="attributesetLeftList">
                        ...
                        {open && openGroupForm()}
                        <Button onClick={deleteAttributeGroupHandler}>Delete Attribute Group</Button>
                        ...
                        <div className="attributeGroup">
                            <ul>
                                {
                                    AttributeGroup.length > 0 && AttributeGroup.map((group, index) => {
                                        return (<li key={`${group.index} + ${index}`} className="group">
                                            <Title
                                                selectedAttributeGroup={() => selectedAttributeGroupHandler(group.index, group.title)} 
                                                title={group.title}
                                                selected={selectedGroup === group.index ? true : false}
                                            />
                                            <ul>
                                                {
                                                    group.attributes.length > 0 && group.attributes.map((attribute, index) => {
                                                        return (attribute.is_done === true && <li key={attribute._id+index}>
                                                            <div className="title">{attribute.frontend_label}</div>
                                                            <div className="icon">
                                                                <ZoomOutMap onClick={() => moveOneGroupToAnother(attribute, group.index)} />
                                                                {attribute.is_user_defined === true && <Delete onClick={() => deleteAttributeFromGroupHandler(attribute, group.index)} />}
                                                            </div>
                                                        </li>)
                                                    })
                                                }
                    ...
                    <div className="attributesetRightList">
                        <ul>
                            {
                                CompletedTodos.length > 0 && CompletedTodos.map((attribute, index) => {
                                    return attribute.is_done === false && <li key={index} className="attribute">
                                        <div className="title">{attribute.frontend_label}</div>
                                        <div className="icon">{attribute.is_done ? <Delete /> : <Done onClick={() => addAttributeToAttributeSetHandler(attribute)} />}</div>
                                    </li>
                                }) 
                            }
                        </ul>
