...
const Attributes = () => {
  ...
  const { loading, error, attributes } = useSelector(
    (state) => state.attributes
  );
  const {
    loading: loadingIsDelete,
    error: deleteError,
    isDeleted,
  } = useSelector((state) => state.deleteAttribute);
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <>
            <Link to={`/admin/attribute/${params.getValue(params.id, "id")}`}>
              <Edit />
            </Link>
            {value && (
              <Button
                onClick={() =>
                  deleteAttributeHandler(params.getValue(params.id, "id"))
                }
              >
                <Delete />
              </Button>
  ...
  return (
    <FormContainer pagetitle={"Attributes Listing"}>
      {loading || loadingIsDelete ? (
        <Loader />
      ) : (
        <>
          <AddNewItemAction
            actionUrl="/admin/attribute/new"
            title={"Add Attribute"}
          />
