...
const NewProduct = () => {
  ...
  const { loading: loadingAttributeSet, attributesets } = useSelector(
  ...
  useEffect(() => {
    ...
    if (!attributeSetId) {
      setAttributeSetId({
        label: attributesets[0].attribute_set_name,
        value: attributesets[0]._id,
      });
      ...
      const attribute_group = attributesets.find(
        (item) => item._id === attributeSetId.value
      ).attribute_group;
  ...
  const checkboxOptionChangeHandler = (event, index, identifier) => {
    ...
    if (event.target.checked) {
      const isExist = chkValue.find(
        (item) => item === event.target.value.trim()
      );
      if (!isExist) {
  ...
  const reordeAttributesets = useMemo(() => {
    ...
    for (let key in attributesets) {
      options.push({
        label: attributesets[key].attribute_set_name,
        value: attributesets[key]._id,
      });
    }
    return options;
  }, [attributesets]);
  ...
  return (
    <FormContainer pagetitle={"Create Product"}>
      {loading ||
      loadingAttributeSet ||
      loadingCategories ||
      defaultFormAttributes === undefined ? (
        <Loader />
      ) : (
        <FormAction submitHandler={createProductSubmitHandler}>
          ...
          {formElementArray.map(
            ({ attributeGroupId, attributeGroup, attributes }, index) => {
              return (
                <FieldSet key={attributeGroupId} legend={attributeGroup}>
                      ...
                      case "multiselect":
                        return (
                          <Input
                            ...
                            changed={(e) =>
                              selectOptionChangeHandler(e, index, attribute.id)
                            }
                          />
                        );
                      case "checkbox":
                      ...
                      case "file":
                        return (
                          <Input
                            ...
                            changed={(e) =>
                              createProductImageChange(e, index, attribute.id)
                            }
                          />
                        );
                      default:
                        return (
                          <Input
                            ...
                            touched={attribute.config.touched}
                            changed={(e) =>
                              inputOptionChangeHandler(e, index, attribute.id)
                            }
                          />
                        );
                    }
