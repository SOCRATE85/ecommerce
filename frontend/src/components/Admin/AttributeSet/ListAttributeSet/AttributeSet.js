...
const AttributeSet = () => {
  ...
  const { loading, attributesets } = useSelector(
  ...
  const {
    ...
    isDeleted,
    error: deleteError,
  } = useSelector((state) => state.deleteAttributeset);
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (num > 0) {
          return (
            <>
              <Link to={`/admin/attributeset/${params.row.id}`}>
                <Edit />
              </Link>
              {params.row.attribute_set_name === "Default" ? (
          ...
          return (
            <>
              ...
              {params.row.attribute_set_name === "Default" ? (
                ...
                <Button
                  onClick={() => deleteAttributesetHandler(params.row.id)}
                >
                  <Delete />
                </Button>
              )}
  ...
  return (
    <FormContainer pagetitle={"Manage Attribute sets"}>
      {loading || deleteLoading ? (
        <Loader />
      ) : (
        <>
          <AddNewItemAction
            actionUrl="/admin/attributeset/new"
