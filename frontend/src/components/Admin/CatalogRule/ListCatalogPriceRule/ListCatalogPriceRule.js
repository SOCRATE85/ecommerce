...
const ListCatalogPriceRule = () => {
  ...
  const { catalogrules, error, loading } = useSelector(
    (state) => state.catalogrules
  );
  const {
    isDeleted,
    error: deleteError,
    loading: deleteLoading,
  } = useSelector((state) => state.deleteCatalogRule);
  ...
  const columns = [
    ...
    { field: "to_date", headerName: "End", minWidth: 200, flex: 1 },
    {
      ...
      renderCell: (params) => {
        return (
          <>
            <Link to={`/admin/catalog_rule/${params.row.id}`}>
              <Edit />
            </Link>
  ...
  const catalogruleData = useMemo(() => {
    const rows = [];
    catalogrules &&
      catalogrules.forEach((catalogrule) => {
        rows.push({
          id: catalogrule._id,
