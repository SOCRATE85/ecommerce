...
const ListBlogCategory = () => {
  ...
  const [doGetAllBlogCategory, isLoadingBlogCategory, blogCategoryError] =
    useThunk(getAllBlogCategories);
  const [doDeleteBlogCategory, isLoadingDelete, errorDelete] =
    useThunk(deleteBlogCategory);
  ...
  const columns = [
    ...
    { field: "name", headerName: "Category Name", flex: 1 },
    {
      ...
      cellClassName: (params) => {
        return params.row.status === 1 ? "greenColor" : "redColor";
      },
    },
    {
      field: "actions",
      ...
      renderCell: (params) => {
        return (
          <>
            <Link to={`/admin/blog/category/${params.row.id}`}>
              <Edit />
            </Link>
  ...
  return (
    <FormContainer pagetitle={"Manage Blog Category"}>
      {isLoadingBlogCategory || isLoadingDelete ? (
        <Loader />
      ) : (
        <>
          <AddNewItemAction
