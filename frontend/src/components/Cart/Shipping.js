...
const Shipping = () => {
  ...
  const [shippingSameAsBilling, setShippingSameAsBilling] = useState(
    _shippingSameAsBilling ? _shippingSameAsBilling : true
  );
  ...
  useEffect(() => {
    if (selectedShippingAddress === "newaddress") {
      ...
      if (addresses) {
        const isMatched = addresses.find(
          (item) => item._id === selectedShippingAddress
        );
        if (isMatched) {
          setShippingAddress({
            selectedShippingAddress,
  ...
  useEffect(() => {
    if (selectedBillingAddress === "newaddress") {
      ...
      if (addresses) {
        const isMatched = addresses.find(
          (item) => item._id === selectedBillingAddress
        );
        if (isMatched) {
          setBillingAddress({
            selectedBillingAddress: "",
  ...
  const shippingSubmit = (e) => {
    e.preventDefault();
    if (
      shippingAddress.phoneNo.length < 10 ||
      shippingAddress.phoneNo.length > 10
    ) {
    ...
    if (!_shippingSameAsBilling) {
      if (
        billingAddress.phoneNo.length < 10 ||
        billingAddress.phoneNo.length > 10
      ) {
      ...
      <div className="shippingContainer">
        <div className="shippingBox">
          ...
          >
            ...
            <ControlContainer
              label={"Billing Address same as Shipping Addresses"}
            >
              <input
                type="checkbox"
                defaultChecked={shippingSameAsBilling}
                onClick={(e) => shippingSameAsBillingHandler(e)}
              />
            </ControlContainer>
