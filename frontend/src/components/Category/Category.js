...
const Category = (props) => {
  ...
  const {
    ...
    error: filterError,
  } = useSelector((state) => state.filterSorting);
  const {
  ...
  const updateHandler = (_event, value, key, order) => {
    ...
    switch (flag[1]) {
      ...
      case "radio":
        let _currentValue =
          typeof _filterValues[order][key] === "string"
            ? _filterValues[order][key]
            : "";
        _temp = {};
        if (_currentValue === "") {
          _temp[key] = value;
        } else {
          const isExists = _currentValue
            .split(",")
  ...
  const removeFilter = (value, item, key, order) => {
    ...
    switch (flag[1]) {
        ...
        if (item[key].length > 1) {
          return (
            <span className="filter-value">
              ...
              <Close
                onClick={() => actions(value, item, key, order)}
                fontSize="small"
              />
            </span>
          );
      ...
      case "radio":
        ...
        if (_temp) {
          const selectedValue = _temp.find(
            (_value) => _value._id.toString() === value.toString()
          );
          return (
            <span className="filter-value">
              {selectedValue.value}
  ...
  const getShopByOption = () => {
    ...
    if (params) {
      ...
      return (
        _selectedFilters.length > 0 && (
          <div className="shop-by-option">
                        ...
                        if (flag.length > 1) {
                          return (
                            <li key={key} className="item">
                              ...
                              :{" "}
                              {removeFilter(params[flag[0]], item, key, index)}
                            </li>
                          );
                        } else {
                          return (
                            ...
                            {itemvalue.map((value, i) => {
                              return (
                                <li key={i} className="item">
                                  ...
                                  : {removeFilter(value, item[key], key, index)}
                                </li>
                              );
                            })}
                          </Hoc>
            ...
            <ul className="selected-filters">
              {selectedFilters.map((item, index) => {
                for (let key1 in item) {
                  ...
                  const isExit = Object.keys(
                    isAppliedOnFilter(key1, item[key1])
                  );
                  if (isExit.length) {
                    switch (flag[1]) {
                      case "price":
                        ...
                        if (flag.length > 1) {
                          return (
                            <li key={key1} className="item">
                              <span className="filter-title">{flag[0]}</span>:{" "}
                              {removeFilter(item[key1], item, key1, index)}
                            </li>
                          );
                        } else {
                          return (
                            ...
                            {itemvalue.map((value, i) => {
                              return (
                                <li key={i} className="item">
                                  ...
                                  :{" "}
                                  {removeFilter(value, item[key1], key1, index)}
                                </li>
                              );
                            })}
                          </Hoc>
  ...
  const getOptionValue = (order, key, filters) => {
    ...
    if (location.search) {
      ...
      if (paramValue !== null) {
        ...
        return filterValues.length > 0 && filterValues[order][key]
          ? filterValues[order][key]
          : filters[key];
      }
    } else {
      return filterValues.length > 0 && filterValues[order][key]
              ...
              {filtersorting.map((filters, index) => {
                for (let key1 in filters) {
                  ...
                  switch (flag[1]) {
                      ...
                      if (
                        ...
                        return (
                          <DoubleHandleSlider
                            ...
                            onChange={(e, value) =>
                              updateHandler(e, value, key1, index)
                            }
                            min={filters[key1][0]}
                            max={filters[key1][1]}
                    ...
                    case "radio":
                      return (
                        <li key={key1} className="filter-item">
                          <Typography className="title">
                            {flag[2].replace("@", " ")}
                          </Typography>
                          <div className="filter-container-others">
                            {filters[key1].length > 0 && (
                              <ul className="list">
                                {filters[key1].map((item) => {
                                  return (
  ...
  const getProductGrid = () => {
    ...
    return (
      <>
        ...
        {products.length > 0 && (
          <div className={`productgrid ${gridlist}`}>
            {products.length > 0 ? (
              <>
                {products &&
  ...
  return (
    <>
      <MetaData
        title={`${categoryDetails && categoryDetails.name} -- Ecommerce`}
      />
      <div className="category-container">
        {getCategoryImageSection()}
        <div className="productContainer">
          <div className="products">{getProductGrid()}</div>
          {getFilterBox()}
        </div>
      </div>
    </>
